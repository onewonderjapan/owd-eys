...
const pkg=JSON.parse(await fs.readFile('package.json','utf8')),release='releases/'+pkg.version;
...
// Keep root aliases compatible with older cached HTML, while new pages pin their whole module/data graph.
for(const p of [...produced])if(!p.endsWith('.html')){const dest=path.join(out,release,path.relative(out,p));await fs.mkdir(path.dirname(dest),{recursive:true});await fs.copyFile(p,dest);produced.add(dest);}
let html=(await fs.readFile('src/index.html','utf8')).replace('<!-- WALK_HUD -->',await fs.readFile('src/hud-fragment.html','utf8'));
html=html.replace(/(href|src)="(site\.css|map-walk\.css|fan-notice\.css|main\.js)"/g,(_,attr,name)=>`${attr}="${release}/${name}"`);
html=html.replaceAll('"/vendor/three/','"/'+release+'/vendor/three/');
...
async function module(rel){
 ...
 const pinned=path.join(out,release,'vendor/three',rel);await fs.mkdir(path.dirname(pinned),{recursive:true});await fs.writeFile(pinned,text);produced.add(pinned);
...
await fs.copyFile(path.join(vendor,'LICENSE'),path.join(out,release,'vendor/three/LICENSE.txt'));produced.add(path.join(out,release,'vendor/three/LICENSE.txt'));
...
await fs.mkdir('reports',{recursive:true});await fs.writeFile('reports/build.json',JSON.stringify({version:pkg.version,release_path:release,built_at:new Date().toISOString(),files:rows,bytes:rows.reduce((n,r)=>n+r.bytes,0)},null,2)+'\n');console.log('Build:',rows.length,'files,',rows.reduce((n,r)=>n+r.bytes,0),'bytes');
