...
try{
 ...
 // V7 2026-09-24: the first two rows (10 eager cards) must be painted within 1s
 await page.waitForFunction(()=>[...document.querySelectorAll('.character-card img')].slice(0,10).every(i=>i.complete&&i.naturalWidth>0),null,{timeout:1000}).catch(()=>{});
 const eagerDesktop=await page.evaluate(()=>[...document.querySelectorAll('.character-card img')].slice(0,10).every(i=>i.complete&&i.naturalWidth>0));
 assert.equal(eagerDesktop,true,'first 10 portraits not ready within 1s');
 ...
 await page.setViewportSize({width:390,height:844});await page.goto(new URL('?actor=cast.14&walk=1',url).href);await page.waitForFunction(()=>window.eys?.state().ready,null,{timeout:60000});assert.equal((await state()).walk,null,'URL still lets visitors choose before entering');assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.waitForFunction(()=>[...document.querySelectorAll('.character-card img')].slice(0,10).every(i=>i.complete&&i.naturalWidth>0),null,{timeout:1000}).catch(()=>{});const eagerMobile=await page.evaluate(()=>[...document.querySelectorAll('.character-card img')].slice(0,10).every(i=>i.complete&&i.naturalWidth>0));assert.equal(eagerMobile,true,'first 10 portraits not ready within 1s on 390px');await page.locator('#selected-portrait').evaluate(i=>i.decode());await page.screenshot({path:out+'/selection-mobile.png',fullPage:true});
