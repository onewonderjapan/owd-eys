...
// the shared scaffolding lives in ./ejection/common.js and each style is moving to
// ./ejection/stage-<style>.js with an identical buildStage(ctx) signature. This file
// is the registry; the external export surface (createEjectionStage,
// EJECTION_STYLE_DURATION) is unchanged.
import * as THREE from 'three';
import {IMMERSION_CONFIG} from './immersion-config.js';
import {createEjectionContext, finishStage, tmpVecA, lerp, clamp01, ease, track} from './ejection/common.js';
...
// Each builder configures the shared scene/camera and defines update/reset/begin.
...
// ======================================================== QUICKSAND (E2) ===
function buildQuicksandStage(ctx) {
 const {actors, targetId, isSelf, target, scene, camera, take, eyePos, stage, reduced, poseOnce, lastPose, setHiddenParts} = ctx;
...
// ======================================================== CHANDELIER (E3) ===
function buildChandelierStage(ctx) {
 const {actors, targetId, isSelf, target, scene, camera, take, eyePos, stage, reduced, poseOnce, lastPose, setHiddenParts} = ctx;
...
// ========================================================== BOULDER (E4) ===
function buildBoulderStage(ctx) {
 const {actors, targetId, isSelf, target, scene, camera, take, eyePos, stage, reduced, poseOnce, lastPose, setHiddenParts} = ctx;
...
const STAGE_BUILDERS = {
 ...
 // the original `else` branch: every unknown style plays the flush stage
