...
- **镇民刀人 · 腿 · 拉腿开会**（1.7.0；1.7.1 尸体改官方样式）：镇民中暗藏一位鸭子（UI 绝不揭示，同一颗种子可复现）。鸭子趁无人目击时把镇民刀倒——受害者倒在红斑里（半截身体露出地面，保留角色颜色与帽子，官方样式）。走近倒在红斑里的镇民按 E 或 R（手机点「报警」）触发发现演出并直接进会议，死者椅子空置且票卡红叉不可选。投出普通镇民则本轮继续，投出鸭子则小镇安全并开新一轮；镇民只剩一位且无尸体可报时小镇陷入寂静。非血腥：无血迹、无尖叫、无震屏。
...
| `src/walk-npcs.js` | 漫游镇民闲逛、对话气泡与鸭子刀人/尸体造型（种子确定性，独立加载/释放） |
| `src/walk-round.js` | 1.7.0 镇民轮纯逻辑：鸭子选取、刀人资格、尸体/缺席账本、会议结算（零 Three/DOM） |
