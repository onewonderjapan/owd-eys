...
|---|---|---|
| 原作被刀者尸体以「鹅腿」标识留在原地；社区叫「腿」，报警叫「拉腿」 | 尸体 = 受害者本体上半截（保留角色颜色与帽子/头饰）半埋在扁平卡通红色圆斑里，无腿，无血 | 尸体样式**已由机主截图确认（2026-09-26）：半截身体 + 红色圆斑**；红斑精确尺寸按模型半宽自动算（`poolScale`），图不进 `src/` |
...
- **刀人动作**（`killDuration`）：鸭子面向靶子小步前冲 0.15 m；靶子身体绕自身前后轴倾倒 90°（`visual.rotation.z` 0 → π/2，ease-out）；结束一帧切换成红斑半埋的尸体；鸭子随即选一条 ≥ `fleeDistance` 的路线走开。音频只一声很轻的 `kill:thud`（可静音）。
- **尸体的造型**（1.7.1 起为官方样式，机主截图确认：半截身体 + 红色圆斑；复用受害者已加载的 avatar，零新资产）：
  1. 不翻转、不隐藏身体：仅隐藏名字匹配 `/foot|shin|webbed[\s_]paddle|toe[\s_]seam/i` 的脚部网格（GLTFLoader 会把空格换成 `_`，正则两种都接受；不硬编码 `.080` 等导出尾号），身体、头、眼、喙、帽子与装备照常显示，保留角色自己的颜色。
  2. 倒地动作结束帧解除倾倒（`visual.rotation.z` 归零），再按整只模型的包围盒把鹅垂直下沉，使露出地面的高度 = 模型高度 × `visibleFraction`（0.42）——看上去像身体从红斑里露出一个圆顶，看不到腿。
  3. 红斑：`player` 下加一块扁平圆柱色块，半径 = 模型水平半宽 × `poolScale`（每个角色自动算）、厚 `poolThickness`、贴地，颜色固定 `poolColor`（卡通红）。红斑是平的色块，无飞溅、无血迹纹理；保留原阴影贴片；移除脚下金色光环 marker。
  4. 尸体不再参与闲逛、避让与气泡；`disposeWalkingAvatar` 仍由镇民模块在清除时统一执行（单一所有者，不双重 dispose）。
